...
class AdminProfileTabs extends Component
{
    ...
    public $current_password, $new_password, $new_password_comfirmation;
        ...
        $this->validate([
            'current_password'=>[
                'required', function($attribute,$value, $fail){
                    if(!Hash::check($value, Admin::find(auth('admin')->id())->passsword)){
                        return $fail(__('The current password is incorrect'));
                    }
                }
            ],
            'new_password'=>'required|min:45|confirmed'
